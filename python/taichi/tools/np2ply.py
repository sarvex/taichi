...
class PLYWriter:
    def __init__(self,
        ...
        self.type_map = {
            ply_type: self.corresponding_numpy_types[i]
    ...
    def add_vertex_channel(self, key: str, data_type: str, data: np.array):
        if data_type not in self.ply_supported_types:
            print(f"Unknown type {data_type} detected, skipping this channel")
            return
        if data.ndim == 1:
            ...
            if key in self.vertex_channels:
                print(f"WARNING: duplicate key {key} detected")
            ...
            for i in range(num_col):
                item_key = f"{key}_{str(i + 1)}"
                if item_key in self.vertex_channels:
                    print(f"WARNING: duplicate key {item_key} detected")
    ...
    def add_faces(self, indices: np.array):
        vert_per_face = 3 if self.face_type == "tri" else 4
    ...
    def print_header(self, path: str, _format: str):
        with open(path, "w") as f:
            f.writelines(
                [
                    "ply\n",
                    f"format {_format}" + " 1.0\n",
                    f"comment {self.comment}" + "\n",
                ]
            )
            f.write(f"element vertex {str(self.num_vertices)}" + "\n")
            for i in range(self.num_vertex_channels):
                f.write(
                    (
                        f"property {self.vertex_data_type[i]} {self.vertex_channels[i]}"
                        + "\n"
                    )
                )
            if self.num_faces != 0:
                f.write(f"element face {str(self.num_faces)}" + "\n")
                f.write("property list uchar int vertex_indices\n")
                for i in range(self.num_face_channels):
                    f.write((f"property {self.face_data_type[i]} {self.face_channels[i]}" + "\n"))
            f.write("end_header\n")

    def export(self, path):
        self.sanity_check()
        self.print_header(path, f"binary_{sys.byteorder}_endian")
        with open(path, "ab") as f:
            for i in range(self.num_vertices):
                for j in range(self.num_vertex_channels):
                    f.write(self.vertex_data[j][i])
            vert_per_face = np.uint8(3) if self.face_type == "tri" else np.uint8(4)
    ...
    def export_ascii(self, path):
        ...
        with open(path, "a") as f:
            for i in range(self.num_vertices):
                for j in range(self.num_vertex_channels):
                    f.write(f"{str(self.vertex_data[j][i])} ")
                f.write("\n")
            vert_per_face = 3 if self.face_type == "tri" else 4
            for i in range(self.num_faces):
                f.writelines(
                ...
                for j in range(self.num_face_channels):
                    f.write(f"{str(self.face_data[j][i])} ")
                f.write("\n")
    ...
    def export_frame_ascii(self, series_num: int, path: str):
        ...
        real_path = f"{path}_" + f"{series_num:0=6d}" + ".ply"
